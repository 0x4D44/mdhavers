...
**Repo (HEAD):** `mdhavers` (`d2a88905abdb23aff5ce4228b993bd7d331faaf5`)  
...
- Expanded `src/llvm/coverage_tests.rs` with additional deterministic compile-to-IR drivers (dict-callable path, range/speir expressions, append/shove variants, list-add + string-repeat + `!=`, and global-dict property set) to improve instantiations coverage further (notably `src/llvm/codegen.rs` instantiations **494 → 516**).
- Expanded `src/llvm/coverage_tests.rs` again with a curated builtin-inline call set (abs/min/max/ceil/floor/round/sqrt/pow plus string/list/dict helpers) to further improve instantiations coverage in the unit-crate instance (notably `src/llvm/codegen.rs` instantiations **516 → 552**, missed lines **61 → 60**, missed regions **680 → 676**).
- Expanded `src/llvm/coverage_tests.rs` again with a deterministic compile-to-IR driver for additional list/string/timing builtins, improving `src/llvm/codegen.rs` instantiations **552 → 581** (missed **188 → 159**).
- Expanded `src/llvm/coverage_tests.rs` again with a deterministic higher-order builtin driver (gaun/sieve/reduce/tumble/aw/ony/hunt/find_index/grup_up/pairt_by/ilk/ilkane), improving `src/llvm/codegen.rs` instantiations **581 → 604** (missed **159 → 136**) and raising total instantiation coverage to **91.22%**.
...
- Lexer instantiation hotspot cooling (continued): added a “kitchen sink” lex test to cover additional keyword/operator/string tokenization paths.
- Tri hotspot cooling: added a constructor sweep test that drives all `tri` object kinds and both constructor access paths (`get` and `call`), reducing `src/tri.rs` missed regions.
- LLVM codegen hotspot cooling: added a unit-crate compile-to-IR driver for control-flow + objects (struct/class/try-catch/spread/pipe/match), improving `src/llvm/codegen.rs` instantiation coverage.
...
- Parser hotspot cooling (continued): expanded `tests/parser_error_paths_coverage.rs` with additional error surfaces (including destructure/function/class/struct/match/try-catch/ternary/property/lexer errors) and a single-quote string success case, reducing `src/parser.rs` missed regions **114 → 83**.
...
|---|---:|---:|---:|
| `src/interpreter.rs` | 14,746 / 14,944 | 198 | 98.68% |
| `src/llvm/codegen.rs` | 27,878 / 27,938 | 60 | 99.79% |
