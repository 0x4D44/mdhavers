...
- `make coverage` (writes the same `target/llvm-cov-summary.json` and prints totals)
- `make coverage-gate` (fails if total lines < 98%)
...
- **Lines:** 57,705 / 58,218 = **99.12%** (missed: 513)
- **Functions:** 2,908 / 2,958 = **98.31%** (missed: 50)
- **Regions:** 101,967 / 106,514 = **95.73%** (missed: 4,547)
- **Instantiations:** 4,622 / 5,438 = **84.99%** (missed: 816)
...
- `tests/interpreter_tls.rs`: cover invalid TLS config parsing (invalid CA/server cert/server key).
...
- `src/compiler.rs`, `src/llvm/codegen.rs`, `src/llvm/compiler.rs`, `src/main.rs`: add import-resolution edge tests and reduce “closure-only” uncovered function counts by replacing a few `*_or_else`/`any`/`map_err` closures with non-closure equivalents.
...
|---|---:|---:|---:|
| `src/interpreter.rs` | 13,432 / 13,805 | 373 | 97.30% |
| `src/llvm/codegen.rs` | 29,934 / 30,041 | 107 | 99.64% |
