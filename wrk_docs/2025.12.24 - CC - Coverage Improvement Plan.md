...
- **Lines:** 57,705 / 58,218 = **99.12%**
- **Functions:** 2,908 / 2,958 = **98.31%**
- **Regions:** 101,967 / 106,514 = **95.73%**
- **Instantiations:** 4,622 / 5,438 = **84.99%**
...
- `src/compiler.rs`, `src/llvm/codegen.rs`, `src/llvm/compiler.rs`, `src/main.rs`: reduce “closure-only” uncovered function counts by replacing a few `*_or_else`/`any`/`map_err` closures with non-closure equivalents.
...
- `src/interpreter.rs` (≈373 missed lines)
- `src/llvm/codegen.rs` (≈107 missed lines)
...
- Use `make coverage-gate` to run the canonical command and fail if total lines < 98% (writes `target/llvm-cov-summary.json`).
