...
- `build.rs` no longer writes runtime artifacts into the repo; it emits them into `OUT_DIR` and embeds them via `include_bytes!`.
...
- Eliminating nested `cargo build` usage from `build.rs` (requires a bigger re-think of runtime packaging/linking).
- Repo hygiene decisions (vendored `node_modules/`, `vendor/` provenance/licensing/security posture).
- tri native error spans (avoid `line: 0`) and other UX diagnostics refinements.
