...
## 22:35
- Fixed backend override gating to use standard unix cfgs so the env var forcing is actually compiled in downstream crates.
- Ran `cargo fmt`.
- `cargo build --no-default-features --features cli,llvm,graphics3d` and matching `cargo clippy --all-targets --no-default-features --features cli,llvm,graphics3d` completed cleanly.

## 23:05
- Fixed WGSL shader to use `var lighting` instead of `let mut` (wgsl keyword restriction).
- Ran `cargo fmt`.

## 23:30
- Added debug toggle `MDH_TRI_DEBUG` and improved surface error handling (Outdated/Lost) in tri_engine render paths.
- Fixed BuildStatus guard to avoid borrow conflicts during native builds.
- Ran `cargo fmt` and `cargo build --no-default-features --features cli,llvm,graphics3d`.
- Ran `cargo clippy --all-targets --no-default-features --features cli,llvm,graphics3d` (existing warnings remain).

## 23:55
- Added additional tri debug logging (empty scene/batch hints and scene/camera summary) gated by `MDH_TRI_DEBUG`.
- Ran `cargo fmt`.
- `cargo build --no-default-features --features cli,llvm,graphics3d` completed cleanly.

## 00:40
- Fixed tri runtime to keep GC-visible references by syncing tri object fields into native dicts (prevents child/geometry/material values from being collected).
- Added native field syncing on create/clone/set.
- Ran `cargo fmt` and `cargo build --no-default-features --features cli,llvm,graphics3d`.

## 01:10
- Added tri debug log for first successful draw (items/batches count) under `MDH_TRI_DEBUG`.
- Ran `cargo fmt` and `cargo build --no-default-features --features cli,llvm,graphics3d`.

## 01:35
- Synced parent pointers when adding/removing children in tri runtime.
- Ran `cargo fmt` and `cargo build --no-default-features --features cli,llvm,graphics3d`.

## 01:55
- Added debug log for first render/tick call under `MDH_TRI_DEBUG`.
- Ran `cargo fmt` and `cargo build --no-default-features --features cli,llvm,graphics3d`.

## 02:10
- Added tri debug log for renderer creation under `MDH_TRI_DEBUG`.
- Ran `cargo fmt` and `cargo build --no-default-features --features cli,llvm,graphics3d`.

## 22:47
- Cleaned clippy warnings (thread_local const init, args.first, Default for LoggerCore, Option as_mut, extras.first).
- Removed unused assignment warning in vendored raylib-sys build script.
- Ran `cargo fmt`, `cargo build`, and `cargo clippy --all-targets` (clean).

## 22:54
- Added Wayland failure fallback to X11 event loop creation (auto when no backend specified).
- Added debug log for fallback under `MDH_TRI_DEBUG`.
- Ran `cargo fmt`.

## 23:45
- Added lookAt/luik_at support for numeric x/y/z args (creates native Vec3 target).
- Updated tri showcase scripts to call `kamera.luik_at(0, 0, 0)` so orbiting camera faces origin.
- Ran `cargo fmt`.

## 23:58
- Prefer X11 by default on WSL (even if WINIT_UNIX_BACKEND points at Wayland) to avoid NoCompositor failures.
- Added one-time debug log for first Renderar method call to help trace render path.
- Ran `cargo fmt`.

## 06:55
- Added debug log at entry of `__mdh_tri_rs_call` plus missing-object warning to trace native method calls.
- Ran `cargo fmt`.
