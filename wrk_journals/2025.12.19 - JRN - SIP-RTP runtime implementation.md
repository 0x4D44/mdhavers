...
- Fixed module resolution for stdlib imports when compiling/running from nested directories: interpreter resolves via ancestor/exe search; LLVM codegen canonicalizes source paths and searches ancestors/exe dir. Verified `mdhavers build` succeeds for `examples/showcases/sip_rtp_echo.braw`.
