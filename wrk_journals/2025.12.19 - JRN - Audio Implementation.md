...
## Entry 26
- Fixed MIDI streaming buffer warnings by bypassing raylib-rs update() and calling UpdateAudioStream with proper frame counts.

## Entry 27
- Replaced native runtime audio backend with miniaudio mixer (no raylib), decoding WAV/MP3 into buffers and mixing in a device callback.
- Updated LLVM native linker path to drop embedded raylib and keep only minimal -ldl for miniaudio on Linux.
- Switched runtime dependency to om-fork-miniaudio to avoid bindgen/glibc identifier failures.
- Updated audio docs/README to reflect miniaudio native backend and raylib only for graphics/interpreter.

## Entry 28
- Ported interpreter audio to miniaudio (removed raylib dependency) with shared mixer state and device callback mixing for SFX/music/MIDI.
- Added test-only miniaudio/rustysynth shims and expanded audio unit tests to cover mixing, seek, and state reset paths.
- Updated top-level audio feature to depend on miniaudio (no raylib) and refreshed README/docs audio notes accordingly.
- Ran `cargo fmt` and `cargo check --no-default-features --features audio` to validate the new dependency graph.

## Entry 29
- Aligned new audio unit tests with float comparison clippy expectations.
- Re-ran `cargo fmt` and `cargo check --no-default-features --features audio` after test tweaks.
